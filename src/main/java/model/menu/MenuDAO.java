            ...
            if(ps.executeUpdate()!=prodotti.size())
                ...
                if(rs.next()){
                    ...
                    if(count==0){
                        ...
                        ps.setInt(1,codiceMenu);
                        if(ps.executeUpdate()!=1)
                            throw new InvalidRequestException("Internal error",List.of("Menu non e' stato posto a valido"),HttpServletResponse.SC_BAD_REQUEST);
                    }
                    return true;
                }
                return true;
